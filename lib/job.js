        ...
        var pairs = _(obj).map(
            function(v, k){
                ...
                switch(true){
                    case v === true:
                        val = 'true()'
                    break; case _.isNull(v) || _.isUndefined(v):
                        val = '()'
                    break; case _.isNumber(v):
                        val = v
                    break; default:
                        val = format('"%s"', v)
                    break;
                }
                return format('"%s" := %s', k, val)
            }).join(', ');

        return format('map { %s }', pairs)
...
Job.prototype = {
  ...
  , createdb: function(dbname, input, options){
        if(_.isPlainObject(options)){
            var opt = obj2map(options),
                inp = _.isArray(input) ? input : [input],
                xql = format('db:create("%s", ("%s"), %s)', dbname, inp.join('","'), opt)
            return this.xquery(xql)
        }
  ...
  , render: function(){
        var bindings = _.map(this.bindings, attr).join(','),
            queue = this.modules.map(function(m){
                    return {__cmd__: 'repo-install', path: m}
                }).concat(this.queue)
